...
@if (session('success'))
    ...
    @if (session('teacher'))
        <form style="display: inline-block;" action="{{route('teachers.edit', session('teacher')->id)}}" method="get">
            <button type="submit">Edit</button>
        </form>

        <form style="display: inline-block;" action="{{route('teachers.show', session('teacher')->id)}}" method="get">
            <button type="submit">Show</button>
        </form>

        <form style="display: inline-block;" action="{{route('teachers.destroy', session('teacher')->id)}}" method="post">
            @csrf
            @method('DELETE')
...
>
    <form action="{{route('teachers.store')}}" method="post">
        <div style="padding: 60px;">
            ...
            @foreach (
                \array_merge(
                    (new \App\Http\Requests\StoreTeacherRequest)->rules(),
